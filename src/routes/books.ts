...
const validateBook =[
    ...
    body("authorId").isInt({ min: 1}).withMessage("authorId must be positive integer")
]
...
router.post(
  ...
  (req: Request, res: Response, next: NextFunction) => {
    ...
    if(!errors.isEmpty()) return res.status(400).json({ message: "Invalid author id"})

    const { title, year, authorId } = req.body;
    const authorExists = authors.some(a => a.id === authorId)
    if (!authorExists) return res.status(400).json({ message: "Book already exists for this author" })
